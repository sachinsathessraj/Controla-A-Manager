...
function PremiumFourImagesTextPreview({ data, previewImageUrls, mode }) {
  ...
  const renderImageItem = (idx) => {
    ...
    const finalImageUrl = imageUrl && (imageUrl.startsWith('data:') || imageUrl.startsWith('http')) 
      ...
      : '';
    return (
      <div key={idx} style={mode === 'desktop' ? { flex: 1, minWidth: 180, maxWidth: 220, textAlign: 'left' } : { width: 320, textAlign: 'left', display: 'flex', flexDirection: 'column', alignItems: 'center' }}>
        <div style={{ 
          width: 220, 
          height: 220, 
          margin: '0 auto', 
        ...
        <div style={{ 
          fontSize: mode === 'desktop' ? 12 : 14, 
          color: mode === 'desktop' ? '#0F1111' : '#23272f', 
          width: '100%', 
          textAlign: 'left',
          fontFamily: 'Amazon Ember, Arial, sans-serif',
          marginBottom: mode === 'desktop' ? 0 : 16
        }}>
          {bodies[idx]}
        </div>
      </div>
...
// PremiumThreeImagesTextModule is now imported from a separate file





// Preview component for PremiumThreeImagesTextModule
function PremiumThreeImagesTextPreview({ data, previewImageUrls, mode }) {
  // mode: 'desktop' or 'mobile'
  const images = data.images || ['', '', ''];
  const headlines = data.itemHeadlines || ['', '', ''];
  const bodies = data.itemBodies || ['', '', ''];
  return mode === 'desktop' ? (
    <div style={{ width: '100%', display: 'flex', flexDirection: 'column', alignItems: 'center' }}>
      {data.headline && (
        <div style={{ width: '100%', maxWidth: 1200, margin: '0 auto 8px', padding: '0 20px' }}>
          <div style={{ fontWeight: 700, fontSize: 18, color: '#23272f', fontFamily: 'Amazon Ember, Arial, sans-serif', textAlign: 'center' }}>
            {data.headline}
          </div>
        </div>
      )}
      <div style={{ display: 'flex', gap: 16, justifyContent: 'space-between', alignItems: 'flex-start', width: '100%', maxWidth: 1200, padding: '0 20px' }}>
        {[0,1,2].map(idx => (
          <div key={idx} style={{ flex: 1, textAlign: 'left', display: 'flex', flexDirection: 'column', alignItems: 'center' }}>
            <div style={{ width: '100%', marginBottom: 10, display: 'flex', justifyContent: 'center' }}>
              {previewImageUrls[data.id + '_three_' + idx] ? (
                <img 
                  src={previewImageUrls[data.id + '_three_' + idx]} 
                  alt={headlines[idx] || `Image ${idx+1}`} 
                  style={{ maxWidth: '100%', height: 'auto', maxHeight: 400, objectFit: 'contain' }} 
                />
              ) : null}
            </div>
            <div style={{ fontWeight: 550, fontSize: 17, color: '#0F1111', fontFamily: 'Amazon Ember, Arial, sans-serif', margin: '0 0 10px', width: '100%' }}>
              {headlines[idx]}
            </div>
            <div style={{ fontSize: 14, color: '#0F1111', fontFamily: 'Amazon Ember, Arial, sans-serif', width: '100%' }}>
              {bodies[idx]}
            </div>
          </div>
        ))}
    ...
    <div style={{ width: '100%', display: 'flex', flexDirection: 'column', alignItems: 'center' }}>
      {data.headline && (
        <div style={{ fontWeight: 700, fontSize: 16, margin: '0 0 16px', color: '#23272f', textAlign: 'center', width: '100%', maxWidth: 1200, padding: '0 20px' }}>
          {data.headline}
        </div>
      )}
      <div style={{ display: 'flex', flexDirection: 'column', gap: 24, width: '100%', maxWidth: 480, padding: '0 20px' }}>
        {[0,1,2].map(idx => (
            ...
            <div style={{ marginBottom: 8, display: 'flex', justifyContent: 'center' }}>
              {previewImageUrls[data.id + '_three_' + idx] && (
                <img 
                  src={previewImageUrls[data.id + '_three_' + idx]} 
                  alt={headlines[idx] || `Image ${idx+1}`} 
                  style={{ maxWidth: '100%', height: 'auto', maxHeight: 180, objectFit: 'contain' }} 
                />
              )}
            </div>
            <div style={{ fontWeight: 550, fontSize: 17, color: '#0F1111', margin: '0 0 6px', width: '100%' }}>
              {headlines[idx]}
            </div>
            <div style={{ fontSize: 14, color: '#0F1111', width: '100%' }}>
              {bodies[idx]}
            </div>
          </div>
        ))}
...
// Premium Full Image Preview Component with proper sizing and spacing
function PremiumFullImagePreview({ data, previewImageUrls, mode }) {
  const imageUrl = mode === 'mobile' 
    ? (previewImageUrls[`${data.id}_mobile`] || data.mobileImage || '')
    : (previewImageUrls[`${data.id}_desktop`] || data.desktopImage || '');
...
// Premium Navigation Carousel Editor Component
function PremiumNavigationCarouselModule({ data, onChange, onDelete, onMoveUp, onMoveDown, moduleIndex, modulesLength }) {
  // Initialize panels state with 5 default panels if not provided
  const [panels, setPanels] = useState(() => {
    if (Array.isArray(data?.panels)) return data.panels;
    
    // Create 5 default panels
    const defaultTexts = [
      {
        subHeadline: 'NEW ARRIVALS',
        headline: 'Spring Collection 2024',
        body: '<p>Discover our latest designs and colors for the new season. Limited time offer - 30% off on all new arrivals.</p>'
      },
      {
        subHeadline: 'SUMMER SALE',
        headline: 'Up to 50% Off',
        body: '<p>Refresh your wardrobe with our summer collection. Special prices for a limited time only.</p>'
      },
      {
        subHeadline: 'LUXURY EDITION',
        headline: 'Handcrafted Elegance',
        body: '<p>Experience the finest quality with our exclusive luxury collection. Limited pieces available.</p>'
      },
      {
        subHeadline: 'TRENDING NOW',
        headline: 'Best Sellers',
        body: '<p>Shop our most loved products this season. Customer favorites with rave reviews.</p>'
      },
      {
        subHeadline: 'EXCLUSIVE',
        headline: 'Members Only',
        body: '<p>Join our membership program for exclusive access to sales and new collections.</p>'
      }
    ];
    ...
    return Array(5).fill().map((_, index) => ({
      ...
      subHeadline: defaultTexts[index % defaultTexts.length].subHeadline,
      headline: defaultTexts[index % defaultTexts.length].headline,
      body: defaultTexts[index % defaultTexts.length].body,
      image: '',
      imageAlt: '',
      textColor: '#000000',
  ...
  const handlePanelChange = (panelIndex, field, value) => {
    ...
    updatedPanels[panelIndex] = { 
      ...
      ...(field === 'image' && { imageAlt: value ? `Panel ${panelIndex + 1} background` : '' })
    };
    setPanels(updatedPanels);
    onChange({ ...data, panels: updatedPanels });
  };
  
  // Handle panel reordering
  const handlePanelMove = (fromIndex, direction) => {
    if (fromIndex < 0 || fromIndex >= panels.length) return;
  ...
  const handlePanelRemove = (index) => {
    if (panels.length <= 2) return; // Don't allow removing if only 2 panels left
  ...
  const onCancel = () => {
    // Reset to original panel data or close the editor
    if (activePanel.originalData) {
      const updatedPanels = [...panels];
      updatedPanels[activePanelIndex] = { ...activePanel.originalData };
      onChange({ ...data, panels: updatedPanels });
    }
    // Close the editor or reset state as needed
  };

  const onSave = () => {
    // Save the current panel data
    const updatedPanels = [...panels];
    ...
    // If this is the last panel, close the editor or show success message
    if (activePanelIndex === panels.length - 1) {
      // Handle save completion (e.g., close modal, show success message)
      console.log('All panels saved successfully');
    }
  };
  ...
  const addPanel = () => {
    ...
    const newPanel = {
      id: uuidv4(),
      navigationText: `Panel ${panels.length + 1}`,
      subHeadline: 'Sub Headline',
      headline: 'Main Headline',
      body: '<p>Add your descriptive text here.</p>',
      image: ''
    };
  ...
  const removePanel = (index) => {
    ...
    if (activePanelIndex >= updatedPanels.length) {
      newActiveIndex = Math.max(0, updatedPanels.length - 1);
  ...
  const handleImageUpload = async (file, panelIndex, isMobile = false) => {
    ...
    if (!file.type.match('image.*')) {
      setErrors(prev => ({
        ...prev,
        [`panel-${panelIndex}-image`]: 'Please upload a valid image file (JPEG, PNG, GIF, WebP)'
      }));
      return false;
    }
    ...
    if (file.size > maxSizeMB * 1024 * 1024) {
      setErrors(prev => ({
        ...prev,
        [`panel-${panelIndex}-image`]: `Image size should be less than ${maxSizeMB}MB`
      }));
      return false;
    }
    ...
    return new Promise((resolve) => {
      img.onload = async () => {
        if (img.width < minDims.width || img.height < minDims.height) {
          setErrors(prev => ({
            ...prev,
            [`panel-${panelIndex}-image`]: `${minDims.label} image too small. Min: ${minDims.width}×${minDims.height}px`
          }));
          resolve(false);
          return;
        }
        ...
        setIsUploading(true);
        setErrors(prev => ({ ...prev, [`panel-${panelIndex}-image`]: '' }));
        
        try {
          const filePath = await uploadImageToSupabase(file, 'premium-navigation-carousel');
          const updatedPanels = [...panels];
          updatedPanels[panelIndex] = { 
            ...updatedPanels[panelIndex], 
            image: filePath,
            imageAlt: file.name,
            ...(isMobile && { mobileImage: filePath })
          };
          onChange({ ...data, panels: updatedPanels });
          resolve(true);
        } catch (error) {
          ...
          setErrors(prev => ({
            ...prev,
            [`panel-${panelIndex}-image`]: 'Error uploading image. Please try again.'
          }));
          resolve(false);
        } finally {
          setIsUploading(false);
  ...
  const handleDrop = async (e, panelIndex) => {
    e.preventDefault();
    setIsDragging(false);
    const file = e.dataTransfer.files?.[0];
    if (file) {
      await handleImageUpload(file, panelIndex);
    }
  };
  ...
  const activePanel = panels[activePanelIndex] || {
    id: uuidv4(),
    navigationText: `Panel ${activePanelIndex + 1}`,
    subHeadline: '',
    headline: '',
    body: '',
    image: '',
  ...
  const validateFile = (file, panelIndex) => {
    // Validate file type
    if (!file.type.match('image.*')) {
      setErrors(prev => ({
        ...prev,
  ...
  const handleImageChange = (index, updatedPanel) => {
    const updatedPanels = [...panels];
...
return (
  <div className="premium-navigation-carousel-editor">
    ...
    <div className="editor-content">
      {/* Left Column - Panel Selector */}
      <div className="panel-previews">
        <div className="panel-previews-header">
          <h4>Panels ({panels.length})</h4>
          <button 
            className="btn-add-panel"
            onClick={addPanel}
            disabled={panels.length >= 10}
            title={panels.length >= 10 ? 'Maximum 10 panels allowed' : 'Add panel'}
          >
            <i className="fas fa-plus"></i> Add Panel
          </button>
        </div>
        <div className="panel-previews-grid">
      ...
      {/* Right Column - Panel Editor */}
      <div className="panel-editor">
        <h5>Panel {activePanelIndex + 1}</h5>
        
        <div className="form-group">
          <label>Navigation Text <span className="required">*</span></label>
          <input
            type="text"
            className="form-control"
            value={activePanel.navigationText || ''}
            onChange={(e) => handlePanelChange(activePanelIndex, 'navigationText', e.target.value)}
            placeholder="e.g., Small Dogs, Family Dogs"
            required
          />
          <div className="form-hint">This text appears in the carousel navigation</div>
        </div>
        ...
        <div className="form-group">
          ...
          <div style={{ display: 'flex', alignItems: 'center' }}>
            <input
              type="checkbox"
              checked={activePanel.showCaption !== false}
              onChange={(e) => handlePanelChange(activePanelIndex, 'showCaption', e.target.checked)}
              style={{ marginRight: '8px' }}
            />
            <span>Show caption on carousel slide</span>
          </div>
        </div>
        ...
        <div className="form-actions">
          <div className="color-picker">
            ...
            <div className="color-options">
              {/* Color options will go here */}
            </div>
          </div>
        </div>
      </div> {/* Close panel-editor */}
    </div> {/* Close editor-content */}
  </div> /* Close premium-navigation-carousel-editor */
);
}
      ...
      panels: [
        {
          ...
          subHeadline: 'Sub Headline',
          headline: 'Main Headline',
          body: '<p>Add your descriptive text here with <strong>formatting</strong> and <em>styling</em> options.</p>',
          image: '',
          imageAlt: '',
          textColor: '#000000',
          textPosition: 'right',
        ...
        },
        {
          ...
          subHeadline: 'Another Feature',
          headline: 'Second Panel',
          body: '<p>Highlight different features or products in each panel.</p>',
          image: '',
          imageAlt: '',
          textColor: '#000000',
          textPosition: 'left',
...
function PremiumSingleLeftImagePreview({ data, previewImageUrls }) {
  const imageUrl = previewImageUrls[data.id] || '';
  return imageUrl ? (
    <div style={{ display: 'flex', alignItems: 'flex-start', gap: 32, width: '100%', maxWidth: 900, margin: '0 auto' }}>
      <img src={imageUrl} alt={data.headline || 'Preview'} style={{ width: 300, height: 300, objectFit: 'cover', flexShrink: 0 }} />
...
function StandardSingleImageSidebarPreview({ data, previewImageUrls }) {
  return (
...
function PremiumContentEditPage() {
  ...
  // 1. Add previewMode state, default to 'desktop'
  const [previewMode, setPreviewMode] = useState('desktop');

  // 2. Add useEffect to reset previewMode to 'desktop' when switching to Preview tab
  useEffect(() => {
    if (tab === 'preview') {
      ...
      <div className="acm-module-area" style={{flexDirection: 'column', alignItems: 'flex-start'}}>
        {tab === 'editor' ? (
          ...
          <div style={{ width: '100%', display: 'flex', flexDirection: 'column', alignItems: 'center', background: 'transparent', padding: '0' }}>
            ...
            {previewMode === 'desktop' && (
              modules.length === 0 ? (
                <div key="no-modules-desktop" style={{color: '#64748b', fontSize: '1.05rem', padding: '20px 0' }}>No modules to preview.</div>
              ) : (
                <div key="desktop-preview-container" className="preview-container">
                  {modules.map((mod, idx) => {
            ...
            {previewMode === 'mobile' && (
              modules.length === 0 ? (
                <div key="no-modules-mobile" style={{color: '#64748b', fontSize: '1.05rem'}}>No modules to preview.</div>
              ) : (
                <div key="mobile-preview-container" className="preview-container">
                  {modules.map((mod, idx) => {
                  if (mod && (mod.id === 'premium-regimen-carousel' || mod.type === 'premium-regimen-carousel')) {
                    console.log('Rendering Mobile PremiumRegimenCarousel with data:', {
                      mod: { ...mod, panels: mod.panels ? `${mod.panels.length} panels` : 'no panels' },
                      previewImageUrls: previewImageUrls ? Object.keys(previewImageUrls) : 'No previewImageUrls',
                      modId: mod.id,
                      idx
                    ...
                    // Ensure panels array exists and has items for mobile
                    const previewData = {
                      ...mod,
                      panels: Array.isArray(mod.panels) && mod.panels.length > 0 
                        ? mod.panels 
                    ...
                    return (
                      <div className="module-preview" style={{ maxWidth: '375px', margin: '0 auto', width: '100%' }}>
                        <PremiumRegimenCarouselView 
                          key={mod.id || `regimen-mobile-${idx}`}
                          data={previewData}
                          previewImageUrls={previewImageUrls}
                          mode="mobile"
                        />
