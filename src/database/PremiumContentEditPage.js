...
import PremiumNavigationCarouselModule from '../components/PremiumNavigationCarouselModule';
import PremiumFullImagePreview from '../components/PremiumFullImagePreview';
import PremiumThreeImagesTextPreview from '../components/PremiumThreeImagesTextPreview';
        ...
        if (mod.id && (mod.id === 'premium-navigation-carousel' || mod.type === 'premium-navigation-carousel')) {
          await Promise.all(
            (mod.panels || []).map(async (panel, panelIdx) => {
              try {
                if (panel.image) {
                  if (typeof panel.image === 'object' && panel.image.url) {
                    // Handle object format with url property
                    if (panel.image.url.startsWith('blob:') || panel.image.url.startsWith('data:')) {
                      urls[`${mod.id}_panel_${panelIdx}`] = panel.image.url;
                    } else {
                      const imgUrl = await acmDB.getImage(panel.image.url);
                      urls[`${mod.id}_panel_${panelIdx}`] = imgUrl || panel.image.url;
                    }
                  } else if (typeof panel.image === 'string') {
                    // Handle direct string URL
                    if (panel.image.startsWith('blob:') || panel.image.startsWith('data:')) {
                      urls[`${mod.id}_panel_${panelIdx}`] = panel.image;
                    } else {
                      const imgUrl = await acmDB.getImage(panel.image);
                      urls[`${mod.id}_panel_${panelIdx}`] = imgUrl || panel.image;
                    }
                  }
                } else {
                  urls[`${mod.id}_panel_${panelIdx}`] = '';
                }
              } catch (error) {
                console.error(`Error loading image for panel ${panelIdx}:`, error);
                urls[`${mod.id}_panel_${panelIdx}`] = '';
              }
            })
          );
