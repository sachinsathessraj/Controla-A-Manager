...
// Base styles that don't depend on component state
const baseStyles = {
  container: {
...
const PremiumBackgroundImageModule = ({
  ...
  const handleImageUpload = async (file, isMobile = false) => {
    ...
    try {
      ...
      const tempData = { ...moduleData, [field]: previewUrl };
      ...
      const updatedData = { ...moduleData, [field]: imageUrl };
      setModuleData(updatedData);
      ...
      const revertedData = { ...moduleData, [field]: '' };
...
// Define base styles that don't depend on component state
const baseStyles = {
  container: {
    border: '1px solid #e2e8f0',
    borderRadius: '8px',
  ...
  removeButton: {
    ...
    // Define dynamic styles based on component state
  const containerStyle = {
    ...baseStyles.container,
    backgroundImage: moduleData.backgroundImage ? `url(${moduleData.backgroundImage})` : 'none',
    backgroundColor: moduleData.backgroundImage ? 'transparent' : 'white'
  };

  const contentStyle = {
    ...baseStyles.content,
    color: moduleData.fontColor || '#ffffff',
    textAlign: moduleData.textAlignment || 'left'
  };
    ...
    <div style={containerStyle}>
      <div style={baseStyles.header}>
        <h3 style={baseStyles.headerTitle}>
          ...
          <span style={baseStyles.aiBadge}>AI Ready</span>
        </h3>
        <div style={baseStyles.headerActions}>
          {moduleIndex > 0 && (
            <button onClick={onMoveUp} style={baseStyles.actionButton} title="Move up">
              ↑
            </button>
          )}
          {moduleIndex < modulesLength - 1 && (
            <button onClick={onMoveDown} style={baseStyles.actionButton} title="Move down">
              ↓
            </button>
          )}
          <button onClick={onDelete} style={baseStyles.deleteButton} title="Delete module">
            ×
          </button>
        </div>
      </div>
